from flight_fare.constant import CONFIG_DIR, ROOT_DIR
...
class DataValidation:
    ...
    def validate_dataset_schema(self) -> bool:
        ...
        try:
            ...
            dict = read_yaml_file(file_path = os.path.join(ROOT_DIR,CONFIG_DIR,'schema.yaml'))['columns']
            ...
            if sorted(train_df.columns.to_list()) == sorted(test_df.columns.to_list()) == sorted(schema_file_columns):
                ...
                airline_yaml_col_value = sorted(read_yaml_file(file_path = os.path.join(ROOT_DIR,CONFIG_DIR,'schema.yaml')['domain_value']['Airline']))
                source_yaml_col_value = sorted(read_yaml_file(file_path = os.path.join(ROOT_DIR,CONFIG_DIR,'schema.yaml')['domain_value']['Source']))
                destination_yaml_col_value = sorted(read_yaml_file(file_path = os.path.join(ROOT_DIR,CONFIG_DIR,'schema.yaml')['domain_value']['Destination']))
                additional_info_yaml_col_value = sorted(read_yaml_file(file_path = os.path.join(ROOT_DIR,CONFIG_DIR,'schema.yaml')['domain_value']['Additional_Info']))
